...
Должно быть: `OPENROUTER_API_KEY=<REDACTED> (не пусто).
...
grep -q ADANOS_API_KEY .env || echo 'ADANOS_API_KEY=<REDACTED> >> .env
...
```bash
echo 'ADANOS_API_KEY=<REDACTED> >> /root/PRD-BOT-ALL/.env
echo 'ADANOS_API_KEY=<REDACTED> >> /root/AGENT-WORLD/.env
...
Пишу Python-скрипт и PowerShell-обёртку для автосохранения чатов в `.cursor/chats/`.

---

## Ассистент

Готово. Скрипт создан, первые **6 чатов** экспортированы и всё запушено на GitHub.

**Ветка:** `26.06.26-PRD-BOT-ALL`  
**Коммит:** `04e8c70`
...
| Файл | Назначение |
|------|------------|
| `scripts/sync_cursor_chats.py` | Основной скрипт (ищет чаты Cursor, пишет `.md`) |
| `scripts/sync_cursor_chats.ps1` | Запуск одной командой в Windows |
| `scripts/register_cursor_chats_sync_task.ps1` | Автозапуск каждые 30 минут |
| `.cursor/chats/archive/*.md` | Сохранённые чаты |
| `.cursor/chats/INDEX.md` | Список всех чатов |
| `.cursor/chats/LAST_HANDOFF.md` | Последний чат — для быстрого старта |
| `.cursor/chats/README.ru.md` | Инструкция |

---

## Как пользоваться

### Сейчас (вручную, перед уходом с ПК)
...
### На домашнем ПК (после 18:00)

```powershell
cd ПУТЬ_К_ВАШЕМУ_РЕПО
git pull
```

Откройте:
- `.cursor/chats/INDEX.md` — список чатов  
- `.cursor/chats/LAST_HANDOFF.md` — последний разговор  
- `.cursor/chats/archive/` — полные тексты

### Автоматически (один раз на каждом ПК)

```powershell
powershell -ExecutionPolicy Bypass -File scripts\register_cursor_chats_sync_task.ps1
```

Каждые **30 минут** чаты сохраняются и, если есть изменения, уходят на GitHub.

**Важно:** задачу нужно поставить и на рабочем, и на домашнем ПК — тогда история собирается с обеих машин.

---

## Как это работает (простыми словами)

Cursor хранит чаты **локально** на диске. Скрипт:
1. Находит папку с чатами этого проекта (`PRD-BOT`)
2. Переводит их в читаемые `.md` файлы
3. Кладёт в репозиторий
4. При `-Push` отправляет на GitHub

На другом компе вы делаете `git pull` и видите вчерашние разговоры в папке `.cursor/chats/`.

---

На домашнем ПК после `git pull` откройте `INDEX.md` — там должен быть в том числе наш сегодняшний разговор про синхронизацию чатов. Если нужно — помогу настроить автозадачу на домашнем компе пошагово.
