...
class EntryEngine:
    ...
    def __init__(self, cfg):
        ...
        self.buy_momentum_guard_min_pct = float(
            cfg.get("entry", "buy_momentum_guard_min_pct", default=self.momentum_guard_min_pct)
        )
        self.sell_momentum_guard_min_pct = float(
            cfg.get("entry", "sell_momentum_guard_min_pct", default=self.momentum_guard_min_pct)
        )
        ...
        self.buy_volume_guard_min_ratio = float(
            cfg.get("entry", "buy_volume_guard_min_ratio", default=self.volume_guard_min_ratio)
        )
        self.sell_volume_guard_min_ratio = float(
            cfg.get("entry", "sell_volume_guard_min_ratio", default=self.volume_guard_min_ratio)
        )
        self.counter_flow_guard_mult = float(
            cfg.get("entry", "counter_flow_guard_mult", default=1.4)
        )
        self.direction_tiebreaker_prob_gap = float(
            cfg.get("entry", "direction_tiebreaker_prob_gap", default=0.10)
        )
        self.exhaustion_max_streak = int(
            cfg.get("entry", "exhaustion_max_streak", default=5)
        )
        self.contra_trend_oppose_bars = int(
            cfg.get("entry", "contra_trend_oppose_bars", default=9)
        )
    ...
    def generate_signal(
        ...
        # Reject if signed flow contradicts chosen side.
        if self.min_orderflow_imbalance > 0:
            if is_long and float(norm_imb) < self.min_orderflow_imbalance:
                signal.metadata = {
                    "reject_reason": (
                        f"orderflow_direction_mismatch (BUY but imb={float(norm_imb):+.3f} "
                        f"< +{self.min_orderflow_imbalance:.3f})"
                    ),
                    "composite_score": composite,
                    "side": side,
                }
                return signal
            if (not is_long) and float(norm_imb) > -self.min_orderflow_imbalance:
                signal.metadata = {
                    "reject_reason": (
                        f"orderflow_direction_mismatch (SELL but imb={float(norm_imb):+.3f} "
                        f"> -{self.min_orderflow_imbalance:.3f})"
                    ),
                    "composite_score": composite,
                    "side": side,
        ...
        # Check streak in the last 7 candles.
        # A true same-direction streak is more reliable than raw candle count.
        # =====================================================
        if len(klines) >= 7:
            ...
            for k in last_candles:
                ...
                same_dir = (is_long and c_close > c_open) or ((not is_long) and c_close < c_open)
                if same_dir:
                    current_streak += 1
                    max_streak = max(max_streak, current_streak)
                else:
                    current_streak = 0
            if max_streak >= max(1, int(self.exhaustion_max_streak)):
                # 5+ consecutive candles already moved in our direction = exhaustion
                signal.metadata = {
                    "reject_reason": f"exhaustion_guard ({max_streak}/7 max same-dir streak)",
                    "composite_score": composite,
                    "side": side,
                }
                return signal
        ...
        if len(klines) >= 11:
            ...
            for k in last_10:
                ...
                if is_long and c_close < c_open:
                    ...
                    contra += 1
            if contra >= max(1, min(10, int(self.contra_trend_oppose_bars))):
                signal.metadata = {
        ...
        if is_long and of_sell_vol > 0 and of_sell_vol > of_buy_vol * counter_flow_mult:
            ...
            signal.metadata = {
                "reject_reason": (
                    f"counter_flow_guard (BUY but sell_vol {of_sell_vol:.0f} "
                    f"> buy_vol {of_buy_vol:.0f} * {counter_flow_mult:.1f})"
                ),
                "composite_score": composite,
                "side": side,
            }
            return signal
        if not is_long and of_buy_vol > 0 and of_buy_vol > of_sell_vol * counter_flow_mult:
            # Want to SELL but heavy buying — absorption / counter-flow
            signal.metadata = {
                "reject_reason": (
                    f"counter_flow_guard (SELL but buy_vol {of_buy_vol:.0f} "
                    f"> sell_vol {of_sell_vol:.0f} * {counter_flow_mult:.1f})"
                ),
                "composite_score": composite,
                "side": side,
            }
        ...
        if self.momentum_filter and len(klines) >= self.momentum_lookback + 1:
            ...
            buy_threshold = max(self.buy_momentum_guard_min_pct, 0.0)
            sell_threshold = max(self.sell_momentum_guard_min_pct, 0.0)
            if momentum_pct > buy_threshold and is_long and momentum < 0 and composite < self.guard_confidence_bypass:
                signal.metadata = {
                    "reject_reason": (
                        f"momentum_guard (BUY but momentum={momentum:.4f} ({momentum_pct:.2f}%) "
                        f"over {self.momentum_lookback} bars, threshold={buy_threshold:.2f}%)"
                    ),
                    "composite_score": composite,
                    "side": side,
                }
                return signal
            if momentum_pct > sell_threshold and not is_long and momentum > 0 and composite < self.guard_confidence_bypass:
                signal.metadata = {
                    "reject_reason": (
                        f"momentum_guard (SELL but momentum={momentum:+.4f} ({momentum_pct:.2f}%) "
                        f"over {self.momentum_lookback} bars, threshold={sell_threshold:.2f}%)"
                    ),
                    "composite_score": composite,
                    "side": side,
                }
        ...
        if self.volume_filter and len(klines) >= self.volume_lookback + 1:
            ...
            cur_vol = vols[-1]
            min_ratio = self.buy_volume_guard_min_ratio if is_long else self.sell_volume_guard_min_ratio
            min_ratio = max(min_ratio, 0.0)
            if avg_vol > 0 and cur_vol < avg_vol * min_ratio and composite < self.guard_confidence_bypass:
                signal.metadata = {
                    "reject_reason": (
                        f"volume_guard (vol={cur_vol:.0f} < avg{self.volume_lookback}={avg_vol:.0f} * {min_ratio:.2f})"
                    ),
                    "composite_score": composite,
                    "side": side,
                }
        ...
        # If price is not near/in zone, fall back to strongest unmitigated side zone.
        zone_for_extension = active_zone
        if zone_for_extension is None and zone_context is not None:
            zone_for_extension = (
                zone_context.best_long_entry_zone() if is_long else zone_context.best_short_entry_zone()
            )
        if zone_for_extension is not None and self.max_entry_extension_atr > 0 and atr_value > 0:
            if is_long:
                extension = max(0.0, current_price - float(zone_for_extension.high))
            else:
                extension = max(0.0, float(zone_for_extension.low) - current_price)
