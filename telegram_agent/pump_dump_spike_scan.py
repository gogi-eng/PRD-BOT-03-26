...
class SpikeScanConfig:
    ...
    respect_scalp_hours: bool = True

    max_positions_bypass_min_score: int = 0
    ...
    def from_cfg(cls, cfg: Mapping[str, Any]) -> "SpikeScanConfig":
        ...
        return cls(
            ...
            respect_scalp_hours=bool(raw.get("respect_scalp_hours", True)),
...
def spike_scan_allowed_now(cfg: Mapping[str, Any]) -> bool:
    """Spike scan только в scalp_hours_local (местное UTC+offset), если respect_scalp_hours."""
    sc = SpikeScanConfig.from_cfg(cfg)
    if not sc.respect_scalp_hours:
        return True
    from datetime import datetime, timezone

    from prd_agent.strategies.router import _scalp_hours_set
    from prd_agent.time_hours import entry_check_hour, read_timezone_offset

    t = cfg.get("trading") if isinstance(cfg.get("trading"), dict) else {}
    strat_block = t.get("strategies") if isinstance(t.get("strategies"), dict) else {}
    hours_set = _scalp_hours_set(cfg, strat_block if isinstance(strat_block, dict) else {})
    if not hours_set:
        return True
    tz = read_timezone_offset(cfg)
    check_hour = entry_check_hour(datetime.now(timezone.utc).hour, tz)
