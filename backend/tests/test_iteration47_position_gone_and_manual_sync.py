...
- MANUAL SAFE allows trailing_exit, tp_cap, trend_exit for manual positions
...
class TestManualSafeExitBlocking:
    """
    Regression: MANUAL SAFE should still block hard_sl, early_exit for manual positions,
    but allow trailing_exit, tp_cap, trend_exit.
    """
    ...
    def test_manual_safe_allows_trailing_exit(self):
        ...
        allowed_reasons = [ExitReason.TRAILING_EXIT, ExitReason.TP_CAP, ExitReason.TREND_EXIT]
    ...
    def test_manual_safe_allows_trend_exit(self):
        """MANUAL SAFE should allow TREND_EXIT for manual positions."""
        from engine.exit_engine import ExitReason
        
        allowed_reasons = [ExitReason.TRAILING_EXIT, ExitReason.TP_CAP, ExitReason.TREND_EXIT]
        assert ExitReason.TREND_EXIT in allowed_reasons
        print("PASSED: TREND_EXIT allowed for manual positions")
    ...
    def test_manual_safe_code_exists(self):
        ...
        assert "only trailing_exit/tp_cap/trend_exit allowed" in source.lower(), \
