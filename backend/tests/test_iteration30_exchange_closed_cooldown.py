...
4. Regression: early_exit ignore behavior still works
"""
...
class TestConfigHasExchangeClosedIgnore:
    ...
    def test_config_has_early_exit_in_ignore_lists(self):
        """early_exit should still be in both ignore lists (regression)"""
        config_path = os.path.join(os.path.dirname(__file__), '..', '..', 'bot', 'config.yaml')
        with open(config_path, 'r') as f:
            config = yaml.safe_load(f)
        ...
        assert 'early_exit' in cooldown_reasons, f"early_exit not in ignore_loss_cooldown_reasons"
        assert 'early_exit' in consecutive_reasons, f"early_exit not in ignore_consecutive_loss_reasons"
        print(f"PASS: early_exit in both ignore lists")
