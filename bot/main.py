...
class TradingBot(
    ...
    def __init__(self):
        ...
        _sell_thr = self.cfg.get("entry", "sell_entry_threshold", default=None)
        self.sell_entry_threshold = (
            float(_sell_thr) if _sell_thr is not None else float(self.entry_engine.entry_threshold)
        )
        self.entry_engine.sell_entry_threshold = float(self.sell_entry_threshold)
        ...
        self.manual_trade_learner = ManualTradeLearner(_bd, self.cfg)
        ...
        self.manual_allow_bot_close = bool(self.cfg.get("manual_management", "allow_bot_close_manual", default=True))
        self.manual_close_grace_minutes = float(self.cfg.get("manual_management", "close_grace_minutes", default=30.0))
        self.manual_min_close_profit_pct = float(self.cfg.get("manual_management", "min_close_profit_pct", default=0.8))
        self.manual_min_close_r = float(self.cfg.get("manual_management", "min_close_r", default=0.8))
        self.manual_allow_trailing_exit = bool(self.cfg.get("manual_management", "allow_trailing_exit", default=True))
        self.manual_allow_tp_cap = bool(self.cfg.get("manual_management", "allow_tp_cap", default=True))
        self.manual_allow_hard_sl_close = bool(self.cfg.get("manual_management", "allow_hard_sl_close", default=False))
        self.manual_allow_partial_tp = bool(self.cfg.get("manual_management", "allow_partial_tp", default=False))
        self.manual_allow_profit_drawdown_close = bool(
...
class TradingBotAnalyzeEntryMixin:
    ...
    def _quality_gate_in_scalp_session_hours(self) -> bool:
        """True when pump/dump session window is active (local hour = UTC + scalp timezone_offset)."""
        ss = getattr(self, "scalp_strategy", None)
        if ss is None or not bool(getattr(ss, "enabled", False)):
            return False
        try:
            offset = int(getattr(ss, "timezone_offset", 0) or 0)
        except (TypeError, ValueError):
            offset = 0
        now_utc = datetime.now(timezone.utc)
        local_hour = (now_utc.hour + offset) % 24
        try:
            pumps = set(getattr(ss, "pump_hours_local", set()) or set())
            dumps = set(getattr(ss, "dump_hours_local", set()) or set())
        except TypeError:
            return False
        return local_hour in (pumps | dumps)
    ...
    def _passes_signal_quality_gate(self, symbol: str, signal: EntrySignal) -> tuple[bool, str, dict]:
        ...
        is_scalp_signal = str(signal.metadata.get("strategy", "")).lower() == "scalp_session"
        use_scalp_floor = is_scalp_signal or self._quality_gate_in_scalp_session_hours()
        min_confidence_gate = (
            self.scalp_quality_min_confidence if use_scalp_floor else self.quality_gate_min_confidence
        )
        min_expected_edge_gate = (
            self.scalp_quality_min_expected_edge if use_scalp_floor else self.quality_gate_min_expected_edge
        )
        # Relax floor when manual_trade_learner matched (history-aligned boost already applied to confidence).
        relief = float(self.cfg.get("manual_trade_learning", "quality_gate_conf_relief", default=0.0) or 0.0)
        if relief > 0:
            ml = signal.metadata.get("manual_learning_match")
            min_m = float(self.cfg.get("manual_trade_learning", "min_match_score", default=0.55) or 0.55)
            if isinstance(ml, dict):
                try:
                    if float(ml.get("score", 0) or 0) >= min_m:
                        min_confidence_gate = max(0.50, float(min_confidence_gate) - relief)
                except (TypeError, ValueError):
                    pass
...
class TradingBotLifecycleMixin:
    async def run(self):
        ...
        logger.info("=" * 72)
        logger.info(
            f"Entry threshold={self.entry_engine.entry_threshold:.2f} LONG | "
            f"SELL={getattr(self.entry_engine, 'sell_entry_threshold', self.entry_engine.entry_threshold):.2f} | "
        ...
        logger.info(
            ...
            f"adjust_max_pos={'ON' if self.adaptive_recommendations.adjust_max_positions else 'OFF'}, "
            f"adjust_entry_gates={'ON' if self.adaptive_recommendations.adjust_entry_gates else 'OFF'})"
        )
        logger.info(
        ...
        try:
            ...
            while self._running and not self._stop_event.is_set():
                try:
                    ...
                    self.adaptive_recommendations.maybe_resync_max_positions_from_config(self)
                    self.adaptive_recommendations.maybe_resync_entry_gates_from_config(self)
...
# === bot\mixins\meta_stack_mixin.py ===
import asyncio
import logging
import statistics
...
class TradingBotPositionLoopMixin:
    async def _manage_positions(self, exchange_positions: list | None = None) -> float:
        ...
        if sync_pause_remaining > 0:
            ...
            not self.controls.dry_run
            and len(exchange_positions) == 0
            and self.position_manager.count() > 0
        ):
            logger.info(
                "[POSITION_SYNC] Exchange reports zero open positions; reconciling tracked entries vs closed PnL."
            )

        # Reconcile ghosts even when the exchange returns zero rows — previously `if exchange_positions`
        # skipped this entirely when all symbols were flat.
        for symbol in list(self.position_manager.symbols()):
            if sync_pause_remaining > 0:
                continue
        ...
        for symbol in list(self.position_manager.symbols()):
            ...
            if should_exit:
                ...
                if close_result.get("success"):
                    ...
                    error_msg = str(close_result.get("error", "") or "").lower()
                    # Bybit/API often returns literal "Position not found" once the hedge is flat.
                    # Don't wait for 3 failed closes in that case.
                    immediate_exchange_gone = (
                        "position not found" in error_msg
                        or "position size is zero" in error_msg
                        or "position is zero" in error_msg
                    )
                    attempts_needed = 1 if immediate_exchange_gone else 3
                    logger.warning(
                        f"[EXIT FAILED] {symbol} execute_close failed ({fails}/{attempts_needed}): "
                        f"reason={reason.value} error={close_result.get('error', '?')}"
                    )
                    if fails >= attempts_needed:
                        position_gone = (
                            "position not found" in error_msg
...
# === bot\mixins\strategy_presets_mixin.py ===
from bot.trading_bot_imports import *  # noqa: F401,F403
