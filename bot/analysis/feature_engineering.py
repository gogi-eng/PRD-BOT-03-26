...
class FeatureEngineer:
    ...
    def build(self, klines: List[Dict], orderflow, liq_analysis, atr_value: float) -> FeatureBatch:
        ...
        target_level = liq_analysis.target_level
        liq_bias = float(liq_analysis.signal)
        liq_density = liq_analysis.target_density
        liq_distance = liq_analysis.distance_to_target_pct / 100 if liq_analysis.distance_to_target_pct > 0 else 0.02
        ...
        for idx, candle in enumerate(window):
            ...
            sequence.append(
                [
                    ...
                    volume_rel,
                    atr_pct,
                    orderflow.imbalance_score,
                    orderflow.bullish_ratio - 1.0,
                    orderflow.bearish_ratio - 1.0,
                    orderflow.volume_spike - 1.0,
                    liq_bias,
                    dist_to_target,
