...
class TelegramSignalAgent:
    ...
    def _ensure_runtime_controls_defaults(self) -> None:
        """Персистентные флаги (панель + рестарт). Не перетирают кнопки unified ControlBot."""
        from prd_agent.ops.runtime_controls import load_runtime_controls

        self.state["agent_runtime_controls"] = load_runtime_controls(self.state_path.parent)
        rtc = self._runtime_controls_dict()
        rtc.setdefault("pause_all_execution", False)
        rtc.setdefault("signal_only_mode", False)
        sync_yaml = bool(self.agent_cfg.get("runtime_controls_sync_yaml", False))
        if sync_yaml:
            if "channel_auto_execute" not in rtc:
                rtc["channel_auto_execute"] = bool(self.auto_execute)
            if "market_scanner_auto_execute" not in rtc:
